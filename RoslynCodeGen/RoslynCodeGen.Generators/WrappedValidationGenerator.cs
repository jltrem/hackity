    ...
    public class WrappedValidationGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            var targets = context.SyntaxProvider
                .CreateSyntaxProvider(
                    predicate: (syntaxNode, _) => IsCandidateForGeneration(syntaxNode),
                    transform: (context, _) => GetWrapperType(context))
                .Where( item => item != null);

            var compilationAndWrappers = context.CompilationProvider.Combine(targets.Collect());

            context.RegisterSourceOutput(compilationAndWrappers, (context, source) =>
            {
        ...
        private static WrapperTypeInfo? GetWrapperType(GeneratorSyntaxContext context)
        {
            ...
            var expectedAttributeSymbol =
                context.SemanticModel.Compilation.GetTypeByMetadataName("RoslynCodeGen.WrappedValidationAttribute");
        ...
        private static bool ImplementsEquatable(ITypeSymbol typeSymbol)
        {
            return typeSymbol
                .AllInterfaces
                .Any(i => i.Name == "IEquatable" && i is INamedTypeSymbol namedType &&
        ...
        private static void GenerateWrapperCode(SourceProductionContext context, Compilation compilation,
            ImmutableArray<WrapperTypeInfo> wrappers)
        {
